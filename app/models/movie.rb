...
class Movie < ApplicationRecord
  ...
  reduces includes(:categories, :directors, :writers, :actors).references(:categories, :directors, :writers, :actors).with_attached_cover, filters: [
    ->(title:) { where('lower(title) like ?', "%#{title.downcase}%") },
    ->(category:) { tagged_with(category) },
    ->(classification:) { where(classification: classification) },
